"""The data terminal — the data module's text-based user interface (TUI): each asset's raw days and its database,
then one stage of the chain a hand chooses in the menu — download, ingest or status — started through `make`, its
...
# the state table: the asset, its database, one column of day counts per venue of the one definition, the last day
STATE_COLUMNS = ("asset", "database", *(f"{venue} days" for venue in data_config.SOURCE_VENUES), "last day")
...
def _asset_rows(tickers: list[str]) -> list[dict]:
    """One row per asset — whether its database stands, how many day ZIPs each venue's leaf holds and the last day
    ...
    for ticker in tickers:
        ...
        rows.append({"asset": ticker,
                     "database": _present(data_config.research_ohlcv_duckdb(ticker)),
...
def main() -> int:
    ...
    try:
        ...
        databases = sum(1 for row in rows if row["database"] == "yes")
        tui.gum_style(["Data terminal", f"{' '.join(tickers)} · {len(tickers)} assets · {databases} databases"],
                      "CURRENT")
        print()
