...
The assets are `--tickers`, comma-separated — `ASSET=<TICKER>` on the make line names one, its absence the basket
`TICKERS` — and the terminal takes them as they are named: it defines no basket and reads no snapshot. What it
knows of an asset it reads through this module's own `config.py` and `lean.py`, the one grammar of the raw tree:
whether the database `research_ohlcv_duckdb()` names is there, and the day ZIPs `lean_day_zip_paths()` enumerates
in the leaf `raw_symbol_dir()` builds for each venue of `SOURCE_VENUES`. It opens no database — the measurement of
what a database holds is `data-status`'s — and it computes nothing: a count of files and the last of the days they
are named for is presentation.
...
One fact the plan states as the Makefile holds it: `data-download` and `data-status` are basket lines — they run
the whole basket whatever `ASSET` says, and the snapshot `status` writes is one for the basket — while `ingest`
runs the one asset named.
...
`cwd`, no `env`, no timeout. `data-<stage>` runs the stage in a one-off container of the `data` runner, through
`docker compose run --rm`. The terminal names neither the container nor the order of the chain — the Makefile
does.
...
|---|---|---|---|
| the sub-module | the data module's own instrument: its targets are `data-<stage>`, carried by the one Makefile at the root | inside `module_data`, beside the stages it starts; the shape every module's `sub_module_terminal/` shares | it imports the standard library and its own package alone (D19): the venues and the descriptors through `config.py`, the day ZIPs through `lean.py`; it runs on the host's `python3` with gum and no virtual environment, and opens no database |
