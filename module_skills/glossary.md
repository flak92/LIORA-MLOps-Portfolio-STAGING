...
| trials a search ran — the hyper-parameter search's, or the points the coordinate search put through a fit, in all and per loop. Per loop this is the loop's whole exposure: the ledger's lines for a loop that enumerates a grid, and `trials_drawn_by_loop` for one that runs a study inside itself and offers only the point it chose | `HYPERPARAMETER_SEARCH_TRIAL_COUNT` (the ceiling the hyper-parameter search runs to), `HYPERPARAMETER_SEARCH_STARTUP_TRIAL_COUNT` (the trials TPE draws at random before it fits a density — written down rather than left to the library's default), `trial_count`, `trial_count_by_loop`, `trials_drawn_by_loop` | `trial_count`, `trial_count_by_loop`, `trials_drawn_by_loop` | trials | n_trials |
...
| the HPO objective at one trial — the CAGR of the validation path at the threshold the one selection rule would pick, or the mean uniqueness-weighted log-loss under the model's own objective; a trial a champion gate stopped carries the last fold it reported, so every point the search drew reaches the ledger | `trial_metrics()`, `trial_metric_key()`, the return of `objective()` in `module_ml/hpo.py`, logged per trial by `log_trials()` | — (no artifact carries it; the parameters file keeps the chosen point alone) | — (no page shows it) | a second name for the chosen point's value; a trial's objective in an artifact or a snapshot; a null for a pruned trial |
| the HPO objective value at the chosen point, named for what it is | `objective_key()` | `best_cagr_validation_path`, or `best_logloss` under the model's own objective | best F2–F4 path CAGR (`best path CAGR` in the search table) | best_value, score; one key that holds two different quantities |
| what stops a hyper-parameter trial early — two gates, each on the best a fold could still reach over the whole threshold grid among the points clearing the trade floor: an upper bound, so a gate can never discard a trial the search would have kept. Optuna's median pruner is not one of them and is not used: it compares a trial's best value over all its steps with the median of other trials at one step, which with folds as calendar years is not a comparison of like with like | `fold_pruning_bound()`, `optuna.TrialPruned` | — | — | a pruner that compares folds as if they were epochs; a gate on a fold's realised value rather than on what it could still reach |
...
| the score the chosen entry edge threshold stood at, named for what it is under the objective the experiment froze | `selection_score_key()`, `selection_score()` | `selection_score_cagr_validation_path`, or `selection_score_mean_sharpe` under the model's own objective | selection score; `degradation` for the final holdout's CAGR minus the validation path's — presentation arithmetic | one key that holds two different quantities; a score whose name does not say what it measures |
...
| the walk-forward path of the validation folds | `validation_path` with `cagr`, `max_drawdown`, `calmar`, `profit_factor`, `trade_count` | what F2, F3 and F4 earned as one equity — in `<TICKER>_strategy_evaluation.json`, in every trial and proposal of the coordinate search, and in `ml_status.json` beside them |
...
| `<TICKER>_coordinate_search.json` | `module_ml/coordinate_search.py` | `inputs`, `beam`, `champion_trial`, `round_count`, `search_converged`, `path`, `proposals`, `trials_drawn_by_loop` (per loop, the points its generator put through a fit that never became a line — the hyper-parameter loop's study, every trial of it, pruned and completed alike and the point it started from among them; a loop that draws none is absent) — where the search stands at a round boundary, written at the top of each round and nowhere else; present before the ledger's first line |
...
the parameters are tuned for the state they were searched under. The ten others
