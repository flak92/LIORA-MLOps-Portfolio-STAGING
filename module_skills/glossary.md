...
| trials a search ran — the hyper-parameter search's, or the scored states of the coordinate search, in all and per loop | `HYPERPARAMETER_SEARCH_TRIAL_COUNT` (the ceiling the hyper-parameter search runs to), `trial_count`, `trial_count_by_loop` | `trial_count`, `trial_count_by_loop` | trials | n_trials |
...
| `<TICKER>_coordinate_search.json` | `module_ml/coordinate_search.py` | `inputs`, `beam`, `champion_trial`, `round_count`, `search_converged`, `path`, `proposals` — where the search stands at a round boundary, written at the top of each round and nowhere else; present before the ledger's first line |
