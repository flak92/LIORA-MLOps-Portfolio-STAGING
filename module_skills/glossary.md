...
| a round of the coordinate search — one pass of each loop the profile names, in the frozen order; a loop's pass is its families in order, each seeded by the beam the one before it left, so one forward move and one backward move are one pass and not two | `round_count`, `pass_count_by_loop`; `round` and `pass` of a trial | `round_count`, `pass_count_by_loop`, `round`, `pass` | rounds | iterations (the Map's word); a round of boosting, which `num_boost_round` already names; a pass counted per family |
...
| information beyond the prior, `1 − model / prior` — the model's objective, and the one the coordinate search selects on while `SELECTION_OBJECTIVE` names it | `relative_logloss_skill` | `relative_logloss_skill` | skill (`rel. skill`, `val skill F<n>`, `mean val skill`, `holdout skill`, `skill F<n>` in the tables) | accuracy, edge |
...
| a coordinate of a state — one axis the search moves along, with one generator of moves and one answer to what a candidate must recompute; the state is the ordered dict of them, and a trial carries the whole of it, so the beam, the gate, the ranking and the cache never ask which one moved | `LOOP_MODULES`, `state_key()`, `to_state()`; `COORDINATE_SEARCH_ROUND_LOOPS`, `BARRIER_COORDINATE_NAMES` | `loop`, and the coordinate's own keys in a trial | coordinate | a dimension, an axis, a knob; a coordinate named inside `coordinate_search.py` |
| a family — one expansion of the beam; a loop's families are a frozen tuple and a pass runs them in order, each seeded by the beam the one before it left | `FAMILIES` of a coordinate's module | `family` | — | a stage, a step, a phase; a family whose order a profile chooses |
...
| `<TICKER>_coordinate_search.json` | `module_ml/coordinate_search.py` | `inputs`, `trials`, `beam`, `champion_trial`, `round_count`, `pass_count_by_loop`, `trial_count_by_loop`, `search_converged`, `path`, `proposals` — every scored state, the search's own state, rewritten after every scored trial; present once a search has run |
