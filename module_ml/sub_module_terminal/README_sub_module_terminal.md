...
file, and the standards of its screens `../skill_tui_designer.md`, which every terminal of this tree
obeys.

```bash
make ml-terminal ASSET=BTC              # the TUI over one asset (in this repository ASSET= is required)
make ml-terminal                        # the TUI over the basket, at the workspace
STORE_ASSETS_ARTIFACTS_DIR=../store_assets_artifacts STORE_TRIALS_DIR=../store_trials STORE_STATUS_DIR=../store_status python3 -B -m module_ml.sub_module_terminal.terminal --tickers BTC -h   # the actions, the keys, plain output and the exit codes
```
...
that runs, runs through `make`: this repository's Makefile is where a stage is named, the Orchestration Makefile where
its container, the chain after a promotion and the search's detached session are.
