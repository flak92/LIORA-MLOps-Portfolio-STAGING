...
Run in order; `make ml-all` — the Orchestration Makefile's chain, which this repository's Makefile does not carry — runs them, each stage in a one-off container of the `ml` runner, and a single stage is its own `ml-<stage>` target, in the same runner there and in this repository's venv here, or the one-off process above run by hand in a shell that exports those variables (`module_skills/glossary.md` § Stores). The four stages above `status` fan
...
| coordinate search — outside the chain, by a hand | `make ml-coordinate-search`, one process per asset of the basket, `ASSET=<TICKER>` narrowing it to one — detached on the host through the Orchestration Makefile's `tmux-` twin, which requires `ASSET`, or in the foreground here; `make ml-status` after it | `<TICKER>_coordinate_search.json` and its ledger `<TICKER>_coordinate_search_trials.jsonl`; the studies of its `hpo` loop in the trial ledger |
...
At the workspace every stage runs in a one-off container of the `ml` runner: a fanned-out
...
profile, start the search (detached at the workspace, or in the foreground), read the recorded
